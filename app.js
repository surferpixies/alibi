(() => {
  ...
  function trackedFeelingsProfileHtml() {
    ...
    return `<section class="card tracked-feelings-profile"><div class="tracked-feelings-profile-head"><div><p class="eyebrow">Suivi personnalisé</p><h3>😊 Mes ressentis suivis</h3></div><strong>${tags.length}</strong></div><p class="muted small">Seuls les ressentis qui comptent pour toi apparaissent avant et après les repas.</p>${tags.length ? `<div class="tracked-feelings-summary">${tags.map((tag) => `<span>${tag.emoji} ${esc(t(tag.label))}</span>`).join("")}</div>` : `<p class="tracked-feelings-empty">Choisis ce que tu souhaites observer pour simplifier tes saisies.</p>`}<button type="button" class="secondary" id="editTrackedFeelings">${tags.length ? "Modifier mes ressentis" : "Choisir mes ressentis"}</button></section>`;
  }
  ...
  function renderProfile() {
    ...
    const physiologicalCard = $(".physiological-context-card");
    if (physiologicalCard) physiologicalCard.insertAdjacentHTML("afterend", trackedFeelingsProfileHtml());
    decorateSupplementIcons();
  ...
  if ("serviceWorker" in navigator) {
    window.addEventListener("load", async () => {
      try {
        const reg = await navigator.serviceWorker.register("./sw.js?v=3.46.0");
        await reg.update();
